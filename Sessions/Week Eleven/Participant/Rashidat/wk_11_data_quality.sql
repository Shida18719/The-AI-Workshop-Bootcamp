...
SELECT 'WardSiteNotNull' AS rule_name,
...
SELECT FirstName, 
    LastName,
    DateOfBirth, COUNT(*) AS count_dups
FROM Patients
...
-- Your query here:
SELECT 'WardCapaciltyCheck' AS rule_name,
SUM(CASE WHEN Capacity <= 0 OR Capacity > 200 THEN 1 ELSE 0 END) AS violations,
...
ON a.Patients = p.Patients
WHERE Patients IS NULL;
